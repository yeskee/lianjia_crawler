...
def areaDetail(area_no):
	...
	ta_area.all(areaDetailUrl)


def getMessage(text):	

	#定义匹配在售二手房数量和已近90天成交二手房数量的正则表达式
	get_selling_num='class="totalSellCount"><span>(.*?)</span>套</a>'
	#get_deal_no='/" >90天成交(.*?)套</a>'
	#匹配小区编号的正则表达式
	get_area_no='ershoufang/c(.*?)/" class="totalSellCount"'

	#上面定义的两个正则表达式匹配相应数据
	selling_num=re.compile(get_selling_num).findall(text)
	...
	selling_num = list(map(int, selling_num))
	#deal_no = list(map(int, deal_no))

	#根据小区在售房数量和近90天成效数量的分类器
	for i in range(0,len(selling_num)):
		#下面这条语句可以调用函数生成小区详细信息页面的url
		print('')
		areaDetail(area_no[i])
		#整体分支后注释：
		#本分支只参与在售房信息爬取，只有在售不为0，即可调用相关函数
		if((selling_num[i]!=0)):
